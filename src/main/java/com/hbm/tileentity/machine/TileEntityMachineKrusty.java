...
import com.hbm.items.machine.ItemSliceFuel;
...
public class TileEntityMachineKrusty extends TileEntityMachineBase implements IControlReceiver, SimpleComponent, IGUIProvider, IInfoProviderEC, CompatHandler.OCComponent, IFluidCopiable, IFluidStandardTransceiverMK2, IRORValueProvider, IRORInteractive, IEnergyProviderMK2 {
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemStack) {
        if(i >= 0 && i < 4) {
            if(itemStack.getItem() instanceof ItemSliceFuel)
                return true;
        }
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote) {
			...
			if (explosion)
				timer++;
			if (timer > maxTimer) {
				ChunkRadiationManager.proxy.incrementRad(worldObj, xCoord, yCoord, zCoord, 200);
	...
	private int[] getNeighboringSlots(int id) {
		...
		case 1:
			return new int[] { 0, 2, 3 }; // + 0, 3 -2
		case 2:
			return new int[] { 0, 1, 3 }; // + 0, 3 -1
		case 3:
			return new int[] { 0, 1, 2 }; // + 1, 2 -0
		}
		return null;
	}
			...
			if(slots[i].getItem() instanceof ItemSliceFuel) {
				...
				for(byte j = 0; j < neighborSlots.length; j++) {
					...
					if (j != 3 - i) // 3 stands for 4-1, the number of fuel slots
						slotFlux[neighborSlots[j]] += outFlux * 0.25D; // continuos operation (neighbours except those behind the control rod)
				}
				continue;
			}
			slotFlux[i] = 0;
