...
import com.hbm.main.MainRegistry; // ADD
import com.hbm.sound.AudioWrapper; // ADD
...
public class TileEntityPASource extends TileEntityCooledBase implements IGUIProvider, IConditionalInvAccess, IControlReceiver, SimpleComponent, CompatHandler.OCComponent {
	...
	private int audioDuration = 0; // ADD
	private AudioWrapper audio = null; // ADD

	public int lastSpeed = 0; // MOD
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote) {
			...
			// ADD
			audioDuration = MathHelper.clamp_int(audioDuration, 0, (int) (Math.sqrt(this.lastSpeed > 0 ? this.lastSpeed : 0)/2));
			if(audioDuration > 5) {
				if(audio == null) {
	...
	public AudioWrapper createAudioLoop() {
		return MainRegistry.proxy.getLoopedSound("hbm:block.paSource", xCoord, yCoord, zCoord, 2.0F, 10F, 2.0F);
	}
